...
// src/case-file/parse.ts
var validateGroup = (root, errors) => {
...
var validateResolution = (release, errors) => {
...
var validateRelease = (root, errors) => {
...
var validateWorkItem = (root, errors) => {
...
var parseCaseFile = (input) => {
  ...
  return { ok: true, caseFile: input };
};

// src/config/load-config.ts
var import_yaml = __toESM(require_dist2(), 1);
var UNREMOVABLE_HARD_BLOCKED = [".github/workflows/**", "amends.yml"];
var DEFAULT_REVIEW_REQUIRED = [
  "package.json",
  "package-lock.json",
  "pnpm-lock.yaml",
  "yarn.lock",
...
var DEFAULT_MAX_FILES_CHANGED = 10;
var MODES = ["issue-only", "pr", "auto-merge"];
var isMode = (value) => MODES.includes(value);
var defaultConfig = () => ({
  mode: "pr",
...
var readMode = (root, errors) => {
  const value = root["mode"];
  if (value === void 0) return "pr";
  if (!isMode(value)) {
...
var readPathList = (root, key, defaults, errors) => {
...
var withUnremovable = (paths) => [
  ...paths,
  ...UNREMOVABLE_HARD_BLOCKED.filter((required) => !paths.includes(required))
];
var readMaxFilesChanged = (root, errors) => {
  const limits = root["limits"];
  if (limits === void 0) return DEFAULT_MAX_FILES_CHANGED;
  if (!isRecord(limits)) {
  ...
  if (value === void 0) return DEFAULT_MAX_FILES_CHANGED;
...
var parseDocument = (fileContent) => {
  let parsed;
  try {
    parsed = (0, import_yaml.parse)(fileContent);
  } catch (thrown) {
...
var loadConfig = (fileContent) => {
  if (fileContent === void 0) return { ok: true, config: defaultConfig() };
  const { root, error } = parseDocument(fileContent);
  if (error !== void 0) return { ok: false, errors: [error] };
  if (root === void 0) return { ok: true, config: defaultConfig() };
  const errors = [];
  const config = {
...
// src/adapter/parse-result.ts
var USAGE_SOURCES = ["reported", "estimated", "unavailable"];
var requireNumberOrNull = (parent, key, path, errors) => {
  const value = parent[key];
  if (typeof value !== "number" && value !== null) {
    errors.push({ path, reason: missingOr(value, "a number or null") });
  }
};
var validateUsage = (parent, errors) => {
...
import { dirname, resolve, sep as sep2 } from "node:path";
var CONTAINMENT_PROBE_ROOT = resolve(sep2, "amends-containment-probe");
var isCheckoutContainedPath = (path) => {
  const target = resolve(CONTAINMENT_PROBE_ROOT, path);
  return target === CONTAINMENT_PROBE_ROOT || target.startsWith(CONTAINMENT_PROBE_ROOT + sep2);
};
...
var validateArtifactFiles = (parent, errors) => {
...
var validateAdapterResult = (parent, errors) => {
...
var validateRunOutcome = (parent, key, path, errors) => {
...
var validateObservation = (parent, path, errors) => {
...
var isEnoent = (error) => error instanceof Error && error.code === "ENOENT";
var runAdapter = async (invocation, runner) => {
  let outcome;
  try {
...
var UNTRUSTED_OPEN_PREFIX = "<<<UNTRUSTED_DATA";
var UNTRUSTED_CLOSE = "<<<END_UNTRUSTED_DATA>>>";
var UNTRUSTED_BLOCKS_PLACEHOLDER = "{{untrusted_case_file_blocks}}";
var DELIMITER_SEQUENCES = ["<<<untrusted_data", "<<<end_untrusted_data"];
var checkString = (value, path, errors) => {
...
var collectDelimiterCollisions = (value, path, errors) => {
...
var assemblePrompt = (caseFile, template) => {
  const errors = [];
...
// src/pipeline/fix.ts
var readDeclared = async (reader, checkoutPath, path) => {
...
async function runFixStage(request2, deps) {
  ...
  await checkoutRevision(
    {
      runner: deps.runner,
      repoPath: request2.invocation.input.checkout_path,
      env: request2.invocation.env,
      timeoutMs: request2.invocation.timeoutMs
    },
    revision
  );
  const assembled = assemblePrompt(request2.caseFile, request2.promptTemplate);
  if (!assembled.ok) return { kind: "prompt_rejected", errors: assembled.errors };
  await deps.files.write(request2.promptPath, assembled.prompt);
  const outcome = await runAdapter(request2.invocation, deps.runner);
  if (outcome.kind !== "ok") return { kind: "adapter_failed", failure: outcome };
  const checkoutPath = request2.invocation.input.checkout_path;
  ...
  const bundle = {
    ...
    agentIdentity: {
      agent: request2.invocation.command,
      model: request2.invocation.input.model_config.model
    }
  };
  await deps.files.write(request2.bundlePath, JSON.stringify(bundle, null, 2));
...
function globToRegExp(pattern) {
  const body = pattern.replace(REGEX_SPECIALS, "\\$&").replaceAll("**", DOUBLE_STAR).replaceAll("*", "[^/]*").replaceAll(DOUBLE_STAR, ".*");
...
// src/pr/compose-body.ts
var quote = (value) => `\`${value}\``;
var caseFileSummary = (caseFile) => {
...
var artifactSection = (observation) => [
  "## Counterfactual artifact",
  "",
  ...observation.artifactPaths.map((path) => `- ${quote(path)}`)
].join("\n");
var outcomeLabel = (run) => run.passed ? "passed" : `failed with ${quote(run.failureSignature)}`;
var verificationSection = (observation, links) => [
  "## Verification runs",
  "",
...
var tierSection = (tier, observation) => [
  "## Evidence tier",
  "",
...
var agentSection = (identity) => [
  "## Agent",
  "",
  `- Agent: ${quote(identity.agent)}`,
  `- Model: ${quote(identity.model)}`
].join("\n");
var downgradeSection = (annotation) => ["## Autonomy downgrade", "", `> ${annotation}`].join("\n");
var composePrBody = (input) => {
  const sections = [
...
// src/pr/open-pr.ts
var CANDIDATE_LABEL = "candidate";
var HUMAN_REVIEW_LABEL = "human-review-required";
var humanReviewSection = (paths) => [
  "## Human review required",
  "",
  "This diff touches review-required paths (\xA78.1) and is never auto-merge eligible, regardless of evidence tier:",
  ...paths.map((path) => `- \`${path}\``)
].join("\n");
var commentOnWorkItem = async (request2, client) => {
  if (request2.workItem.kind !== "github_issue") {
    return {
      kind: "invalid_work_item",
      reason: `work_item.kind '${request2.workItem.kind}' is not a GitHub issue`
    };
  }
  if (!/^\d+$/.test(request2.workItem.id)) {
    return {
      kind: "invalid_work_item",
      reason: `work_item.id '${request2.workItem.id}' is not an issue number`
    };
  }
  const issueNumber = Number(request2.workItem.id);
  await client.createComment({ issueNumber, body: request2.body });
  return { kind: "issue_commented", issueNumber };
};
var openFixPr = async (request2, client) => {
  if (request2.autonomy === "issue_only") {
    return commentOnWorkItem(request2, client);
  }
  const body = request2.classification.kind === "review_required" ? `${request2.body}

${humanReviewSection(request2.classification.paths)}` : request2.body;
  await client.createBranchAndPush({
    branch: request2.branch,
    commitMessage: request2.title,
    paths: request2.stagePaths
  });
  const pr = await client.openPullRequest({
    title: request2.title,
    body,
    head: request2.branch,
    base: request2.base
  });
  const labels = [];
  if (request2.tier === 1) {
...
// src/tier/classify.ts
var BUILD_TYPECHECK_RUNNERS = /* @__PURE__ */ new Set(["tsc", "typecheck", "build", "compile"]);
var strongTier2Signals = (observation) => {
  const signals = [];
...
var weakTier1Reasons = (observation) => {
  const reasons = [];
  if (observation.httpExercised && !observation.serverProcessSpawned) {
...
// src/tier/resolve-autonomy.ts
var MODE_CEILINGS = {
  "issue-only": "issue_only",
  pr: "normal_pr",
  "auto-merge": "automerge_eligible"
};
var downgrade = (autonomy, mode, tier) => ({
  autonomy,
  downgraded: true,
...
var PATCH_SCRATCH_PATH = ".git/amends-fix.patch";
var applyFixDiff = async (request2, runner, files) => {
  if (request2.fixDiff.trim() === "") return { applied: true };
  await files.write(join2(request2.repoPath, PATCH_SCRATCH_PATH), request2.fixDiff);
...
var NUMSTAT_ENTRY = /^(?:\d+|-)\t(?:\d+|-)\t([\s\S]*)$/;
var parseNumstatOutput = (raw) => {
  ...
  for (let index = 0; index < tokens.length; index += 1) {
    ...
    if (path === "") {
      ...
      if (oldPath === void 0 || oldPath === "" || newPath === void 0 || newPath === "") {
        return void 0;
      }
...
var SUMMARY_RENAME = /^ (?:rename|copy) (.+) \(\d+%\)$/;
var SUMMARY_MODE_CHANGE = /^ mode change \d+ => \d+ (.+)$/;
var expandSummaryArrow = (compact) => {
...
var reconcileSummary = (summaryRaw, paths) => {
  for (const line of summaryRaw.split("\n")) {
    ...
    if (modeChanged !== void 0 && !paths.has(modeChanged)) {
      return `mode change on unenumerated path: ${modeChanged}`;
    }
  }
  return void 0;
};
var enumerateFixDiffPaths = async (request2, runner, files) => {
  if (request2.fixDiff.trim() === "") return { ok: true, paths: [] };
  await files.write(join2(request2.repoPath, PATCH_SCRATCH_PATH), request2.fixDiff);
  const gitApply = (flags) => runner.run({
    command: "git",
  ...
  if (mismatch !== void 0) return { ok: false, reason: mismatch };
  return { ok: true, paths: [...paths] };
};

// src/pipeline/publish.ts
var BRANCH_REF_PATTERN = /^[A-Za-z0-9][A-Za-z0-9._/-]*$/;
async function materializeFix(request2, deps) {
  const apply = await applyFixDiff(
    {
      repoPath: request2.repoPath,
      fixDiff: request2.fixBundle.fixDiff,
      env: request2.env,
      timeoutMs: request2.timeoutMs
    },
    deps.runner,
  ...
  for (const [path, content] of Object.entries(request2.fixBundle.artifactFiles)) {
    await deps.files.write(join3(request2.repoPath, path), content);
  }
}
var TITLE_FIELD_MAX = 64;
var titleField = (value) => {
...
var composePrTitle = (caseFile) => `Amends: evidence-backed fix for ${titleField(caseFile.work_item.kind)} ${titleField(caseFile.work_item.id)}`;
async function recheckGuardrails(request2, deps) {
  ...
  const classification = classifyDiffPaths(enumerated.paths, request2.config);
...
async function runPublishStage(request2, deps) {
  const verdict = request2.verifyBundle.verdict;
  ...
  const recheck = await recheckGuardrails(request2, deps);
  if (recheck.kind === "refused") return recheck.result;
  const classification = recheck.classification;
  const branch = request2.fixBundle.adapterResult.branch_ref;
  if (!BRANCH_REF_PATTERN.test(branch)) {
    return { kind: "invalid_branch_ref", branchRef: branch };
  }
  if (autonomy !== "issue_only") {
    await materializeFix(request2, deps);
  }
  const body = composePrBody({
    caseFile: request2.caseFile,
    observation: verdict.observation,
    tier,
    autonomy: resolution,
    verificationRunLinks: request2.verificationRunLinks,
    agentIdentity: request2.fixBundle.agentIdentity
  });
  const outcome = await openFixPr(
    {
      ...
      tier: tierLevel,
      classification,
      workItem: request2.caseFile.work_item,
      branch,
      base: request2.base,
      title: composePrTitle(request2.caseFile),
      body,
      stagePaths: [
        .../* @__PURE__ */ new Set([...recheck.paths, ...Object.keys(request2.fixBundle.artifactFiles)])
      ]
    },
    deps.github
  );
...
var summarizePipelineResult = (result) => {
  switch (result.kind) {
    case "published":
...
// src/verification/counterfactual.ts
import { join as join4 } from "node:path";

// src/guardrails/environment-invariance.ts
var VERIFICATION_CONFIG_SET = [
  ".github/workflows/**",
  "amends.yml",
  "tsconfig*.json",
  "vitest.config.*",
...
function checkInvariance(diff, verificationConfigSet) {
...
// src/verification/counterfactual.ts
function checkGuardrails(fixPaths, artifactPaths, config) {
...
var resetToOriginal = (request2, deps) => checkoutRevision(
  { runner: deps.runner, repoPath: request2.repoPath, env: request2.env, timeoutMs: request2.timeoutMs },
  request2.originalRevision
);
async function writeArtifacts(request2, deps) {
  for (const [path, content] of Object.entries(request2.artifactFiles)) {
    await deps.files.write(join4(request2.repoPath, path), content);
  }
}
async function executeTestRun(request2, deps) {
  const result = await runCommand(
    request2,
    deps,
    request2.testCommand.command,
    request2.testCommand.args
  );
...
var buildObservation = (request2, artifactPaths, originalRun, patchedRun) => ({
  runner: request2.runnerName,
  artifactPaths,
...
function insufficiencyReasons(originalRun, patchedRun, apply) {
  const reasons = ["artifact_failed_on_patched"];
  if (!apply.applied) reasons.push("fix_diff_apply_failed");
  if (!originalRun.passed && !patchedRun.passed && originalRun.failureSignature === patchedRun.failureSignature) {
    reasons.push("failure_signature_unchanged_from_original");
  }
  return reasons;
}
async function runCounterfactual(request2, deps) {
  const enumerated = await enumerateFixDiffPaths(
    { repoPath: request2.repoPath, fixDiff: request2.fixDiff, env: request2.env, timeoutMs: request2.timeoutMs },
    deps.runner,
    deps.files
  );
  if (!enumerated.ok) {
  ...
  const artifactPaths = Object.keys(request2.artifactFiles);
  const refusal = checkGuardrails(enumerated.paths, artifactPaths, request2.config);
  if (refusal) return refusal;
  await resetToOriginal(request2, deps);
  await writeArtifacts(request2, deps);
  const originalRun = await executeTestRun(request2, deps);
  ...
  if (apply.applied) {
    await writeArtifacts(request2, deps);
    patchedRun = await executeTestRun(request2, deps);
  } else {
    patchedRun = { passed: false, failureSignature: apply.failureSignature };
  }
  const observation = buildObservation(request2, artifactPaths, originalRun, patchedRun);
...
var parseTestCommand = (caseFile) => {
  const raw = caseFile.validation?.["test_command"];
  if (typeof raw !== "string" || raw.trim() === "") return void 0;
  const [command, ...args] = raw.trim().split(/\s+/);
  return command === void 0 ? void 0 : { command, args };
};
async function verifyVerdict(request2, deps) {
  const { caseFile, fixBundle } = request2;
  ...
  if (testCommand === void 0) missing.push("validation.test_command");
  if (testCommand === void 0 || missing.length > 0) {
  ...
  return runCounterfactual(
    {
      ...
      env: request2.env,
      timeoutMs: request2.timeoutMs,
      config: request2.config
    },
    deps
  );
}
async function runVerifyStage(request2, deps) {
  const verdict = await verifyVerdict(request2, deps);
...
// src/utils/env.ts
var ZERO_SECRET_ENV_ALLOWLIST = ["PATH", "HOME"];
var buildZeroSecretEnv = (source) => {
  const env = {};
...
var DEFAULT_COMMIT_IDENTITY = {
  name: "amends[bot]",
  email: "amends[bot]@users.noreply.github.com"
};
var buildPublishGitEnv = (source, identity) => ({
...
var FIX_STAGE_ENV_PREFIXES = ["LC_", "XDG_"];
var buildFixStageEnv = (source, secretKeys) => {
  const allowed = (key) => FIX_STAGE_ENV_ALLOWLIST.includes(key) || FIX_STAGE_ENV_PREFIXES.some((prefix) => key.startsWith(prefix)) || secretKeys.includes(key);
  const env = {};
...
// src/index.ts
var STAGES = ["fix", "verify", "publish"];
var DEFAULT_TIMEOUT_MS = 6e5;
var readActionInputs = (env) => {
  const input = (name) => {
  ...
  const timeoutMs = timeoutValue === void 0 ? DEFAULT_TIMEOUT_MS : Number(timeoutValue);
  if (!Number.isFinite(timeoutMs) || timeoutMs <= 0) {
    errors.push({ path: "timeout-ms", reason: "expected a positive number of milliseconds" });
  }
  if (errors.length > 0 || stage === void 0 || workspace === void 0 || caseFile === void 0) {
    return { ok: false, errors };
  }
  const inWorkspace = (path) => resolve2(workspace, path);
  return {
    ok: true,
    inputs: {
      stage,
      ...
      promptPath: resolve2(env["RUNNER_TEMP"] ?? tmpdir(), input("prompt-path") ?? "amends/prompt.md"),
      adapterCommand,
      adapterArgs: (input("adapter-args") ?? "").split(/\s+/).filter((arg) => arg !== ""),
      adapterSecretEnv: (input("adapter-secret-env") ?? "").split(",").map((key) => key.trim()).filter((key) => key !== ""),
      model,
...
var EntryFault = class extends Error {
...
var loadFixBundleAt = async (path) => {
...
var loadVerifyBundleAt = async (path) => {
...
var dispatchFix = async (inputs, env) => {
  const caseFile = await loadCaseFileAt(inputs.caseFilePath);
  const template = await readFile2(resolve2(import.meta.dirname, "../prompts/fix-pass.md"), "utf8");
...
var dispatchPublish = async (inputs, env) => {
  const repoFull = requireEnv(env, "GITHUB_REPOSITORY");
  const [owner, repo] = repoFull.split("/");
  if (owner === void 0 || repo === void 0 || repo === "") {
  ...
  const github = createOctokitGitHubClient({
    octokit: new Octokit2({ auth: requireEnv(env, "GITHUB_TOKEN") }),
...
if (isDirectInvocation(process.argv[1])) {
  runAction(process.env).then((outcome) => {
